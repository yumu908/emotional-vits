...
def cjke_cleaners(text):
    ...
    for chinese_text in chinese_texts:
        cleaned_text = chinese_to_lazy_ipa(chinese_text[4:-4])
        cleaned_text = (
            cleaned_text.replace("ʧ", "tʃ").replace("ʦ", "ts").replace("ɥan", "ɥæn")
        )
        text = text.replace(chinese_text, cleaned_text + " ", 1)
    for japanese_text in japanese_texts:
        cleaned_text = japanese_to_ipa(japanese_text[4:-4])
        cleaned_text = (
            cleaned_text.replace("ʧ", "tʃ")
            .replace("ʦ", "ts")
            .replace("ɥan", "ɥæn")
            .replace("ʥ", "dz")
        )
...
def chinese_dialect_cleaners(text):
    text = re.sub(
        r"\[MD\](.*?)\[MD\]", lambda x: chinese_to_ipa2(x.group(1)) + " ", text
    )
    text = re.sub(
        r"\[TW\](.*?)\[TW\]", lambda x: chinese_to_ipa2(x.group(1), True) + " ", text
    )
    text = re.sub(
        r"\[JA\](.*?)\[JA\]",
